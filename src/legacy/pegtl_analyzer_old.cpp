...
Result<AnalysisResult> PEGTLAnalyzer::analyze(const std::string& content, 
    ...
    try {
        ...
        if (language == Language::UNKNOWN) {
            ...
            std::cerr << "[DEBUG] Auto-detected language: " << static_cast<int>(language) << std::endl;
        }
        ...
        switch (language) {
            ...
            case Language::PYTHON:
                result = extract_python_elements(content);
                std::cerr << "[DEBUG] Python analysis: " << result.classes.size() << " classes, " 
                          << result.functions.size() << " functions" << std::endl;
                break;
            default:
...
AnalysisResult PEGTLAnalyzer::extract_python_elements(const std::string& content) {
    ...
    std::cerr << "[DEBUG] extract_python_elements called, content size: " << content.size() << std::endl;
    
    // 🐍 Python解析（簡易正規表現ベース実装）
    // TODO: 将来的にPEGTL文法で実装
    ...
    while (class_iter != end) {
        ClassInfo class_info;
        class_info.name = (*class_iter)[1].str();
        class_info.start_line = calculate_line_number(content, class_iter->position());
    ...
    std::regex import_patterns[] = {
        std::regex(R"(^from\s+([\w.]+)\s+import)", std::regex::multiline),
        std::regex(R"(^import\s+([\w.]+))", std::regex::multiline)
    };
    ...
    for (const auto& pattern : import_patterns) {
        ...
        while (iter != end) {
            ...
            import_info.type = ImportType::ES6_IMPORT; // Pythonなので型を追加すべきだが暫定
...
Language PEGTLAnalyzer::detect_language_from_content(const std::string& content, const std::string& filename) {
    ...
    if (has_suffix(filename, ".py") || has_suffix(filename, ".pyw") || has_suffix(filename, ".pyi")) {
    ...
    if (content.find("def ") != std::string::npos || content.find("class ") != std::string::npos ||
